...
def train_test_split_columns(input_pickle: pd.DataFrame, column_list: dict, label: dict) -> dict:
    ...
    Then returns a dictionary with the column name as a key and a tuple with X_train, X_test, y_train, y_test as value.
    Args:
...
def build_array_dic(splits_dic: dict) -> dict:
    ...
    Function that takes the dictionary generated by "train_test_split_columns" and reshapes X_train to fit a CNN model. \
         Then builds a 1D CNN model using the function 'build_selection_model'. Returns both the reshaped arrays and the models.
...
def determine_best_fp(input_pickle: pd.DataFrame, column_list: list, label: dict) -> dict:
    """
    Function that splits and reshape the data of several columns (determined by input) so a CNN model can be build for each of these \
        columns (each corresponding to a different type of fingerprint). The accuracy of each model is calculated and returned in a \
        dictionary.
    Args: 
      input_pickle: input pandas dataframe.
...
def prepare_data(input_pickle: pd.DataFrame, fingerprints_accuracies: dict, label: dict) -> tuple:
    ...
    Args: 
      input_pickle: input pandas dataframe.
      fingerprints_accuracies: a dictionary with column names as keys and the accuracy obtained by the CNN model for each column as \
        values.
      label: name of the column selected as target values (determined in Kedro's parameters).
    Output:
